...
CREATE TABLE CONCEPTO_TRIBUTARIO (
    ...
    VIGENCIA_HASTA          DATE,
    ACTIVO                  NUMBER(1)     DEFAULT 1,
    CONSTRAINT FK_CT_PADRE  FOREIGN KEY (CODIGO_PADRE) REFERENCES CONCEPTO_TRIBUTARIO(CODIGO)
);
...
CREATE TABLE SECCION_PLANTILLA (
    ...
    CONFIG_LAYOUT_JSON      CLOB CONSTRAINT CK_SEC_JSON CHECK (CONFIG_LAYOUT_JSON IS JSON),
    VISIBLE_DEFECTO         NUMBER(1)     DEFAULT 1,
    CONSTRAINT FK_SEC_VER   FOREIGN KEY (CODIGO_VERSION_PLANTILLA) REFERENCES VERSION_PLANTILLA(CODIGO),
    CONSTRAINT FK_SEC_PAD   FOREIGN KEY (CODIGO_SECCION_PADRE)     REFERENCES SECCION_PLANTILLA(CODIGO)
);

-- Cubrir la FK de auto-referencia de SECCION_PLANTILLA
CREATE INDEX IDX_SEC_PADRE
    ON SECCION_PLANTILLA(CODIGO_SECCION_PADRE);

-- Índice compuesto para búsqueda por versión + orden (muy frecuente)
CREATE INDEX IDX_SEC_VER_ORDEN
    ON SECCION_PLANTILLA(CODIGO_VERSION_PLANTILLA, ORDEN);

-- Mismo patrón en CAMPO_PLANTILLA
CREATE INDEX IDX_CP_VER_ORDEN
    ON CAMPO_PLANTILLA(CODIGO_VERSION_PLANTILLA, ORDEN);

CREATE TABLE CAMPO_PLANTILLA (
    CODIGO                  NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    CODIGO_VERSION_PLANTILLA NUMBER NOT NULL,
    CODIGO_SECCION          NUMBER,
    -- El widget que se usó de la biblioteca (referencia al origen, no bloquea el snapshot)
    ...
    -- Snapshot de configuracion: default de biblioteca + personalizaciones del diseñador
    PROPIEDADES_UI_JSON     CLOB CONSTRAINT CK_CP_JSON CHECK (PROPIEDADES_UI_JSON IS JSON),
    CONSTRAINT FK_CP_VER    FOREIGN KEY (CODIGO_VERSION_PLANTILLA) REFERENCES VERSION_PLANTILLA(CODIGO),
    CONSTRAINT FK_CP_SEC    FOREIGN KEY (CODIGO_SECCION)           REFERENCES SECCION_PLANTILLA(CODIGO),
    CONSTRAINT FK_CP_BIBLIO FOREIGN KEY (CODIGO_BIBLIOTECA_COMPONENTE) REFERENCES BIBLIOTECA_COMPONENTE(CODIGO),
    CONSTRAINT FK_CP_CAT    FOREIGN KEY (CODIGO_CATALOGO_MAESTRO)   REFERENCES CATALOGO_MAESTRO(CODIGO),
...
CREATE TABLE CAMPO_REGLA_VALIDACION (
    ...
    PARAMETROS_JSON         CLOB CONSTRAINT CK_CRV_JSON CHECK (PARAMETROS_JSON IS JSON),
    PRIMARY KEY (CODIGO_CAMPO_PLANTILLA, CODIGO_REGLA_VALIDACION),
    CONSTRAINT FK_CRV_CAMPO FOREIGN KEY (CODIGO_CAMPO_PLANTILLA)  REFERENCES CAMPO_PLANTILLA(CODIGO),
    CONSTRAINT FK_CRV_REGLA FOREIGN KEY (CODIGO_REGLA_VALIDACION) REFERENCES REGLA_VALIDACION(CODIGO)
);
...
CREATE TABLE REGLA_CONDICIONAL (
    ...
    ORDEN                   NUMBER DEFAULT 0,
    CONSTRAINT FK_RC_CAMPO  FOREIGN KEY (CODIGO_CAMPO_PLANTILLA) REFERENCES CAMPO_PLANTILLA(CODIGO)
);
...
CREATE TABLE REGLA_CONDICIONAL_DETALLE (
    ...
    CONSTRAINT FK_RCD_REGLA FOREIGN KEY (CODIGO_REGLA_CONDICIONAL) REFERENCES REGLA_CONDICIONAL(CODIGO) ON DELETE CASCADE
);
...
CREATE TABLE CAMPO_PLANTILLA_CONCEPTO (
    ...
    CONSTRAINT FK_MCC_CAMPO    FOREIGN KEY (CODIGO_CAMPO_PLANTILLA)      REFERENCES CAMPO_PLANTILLA(CODIGO),
    CONSTRAINT FK_MCC_CONCEPTO FOREIGN KEY (CODIGO_CONCEPTO_TRIBUTARIO)  REFERENCES CONCEPTO_TRIBUTARIO(CODIGO)
);
...
CREATE TABLE DECLARACION_OBSERVACION (
    ...
    CONSTRAINT FK_DO_CAMPO  FOREIGN KEY (CODIGO_CAMPO_PLANTILLA)        REFERENCES CAMPO_PLANTILLA(CODIGO),
...
CREATE TABLE DECLARACION_ADJUNTO (
    ...
    CONSTRAINT FK_DA_CAMPO  FOREIGN KEY (CODIGO_CAMPO_PLANTILLA)        REFERENCES CAMPO_PLANTILLA(CODIGO),
...
INSERT INTO DECLARACION_HISTORIAL_ESTADO (CODIGO_FORMULARIO_DECLARACION, ESTADO_ANTERIOR, ESTADO_NUEVO, COMENTARIO, USUARIO_RESPONSABLE) VALUES
(
